...
CREATE TABLE artista (
    ...
    stileP VARCHAR(256) REFERENCES stile(nome) DEFERRABLE INITIALLY DEFERRED,
...
CREATE TABLE ogg_arte (
    ...
    artista VARCHAR(256) REFERENCES artista(nome) DEFERRABLE INITIALLY DEFERRED,
    stile VARCHAR(256) REFERENCES stile(nome) DEFERRABLE INITIALLY DEFERRED
);

CREATE TABLE prestito (
    ID INTEGER PRIMARY KEY REFERENCES ogg_arte(ID) ON DELETE CASCADE DEFERRABLE INITIALLY DEFERRED,
...
CREATE TABLE ogg_esib (
    nomeEsibizione VARCHAR(256) REFERENCES esibizione(nome) DEFERRABLE INITIALLY DEFERRED,
